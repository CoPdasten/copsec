...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      cat << 'HELP_EOF'
Usage: sudo bash install.sh --role=<controller|collector> [OPTIONS]

Core Roles:
  --role=controller              Provision central Vault, gRPC ingestion, and Web SOC Cockpit
  --role=collector               Provision autonomous Edge Sensor with native eBPF/XDP and Gossip

Options:
  --controller-ip=<ip>          Central Controller IP (Default: 192.168.1.10)
  ...
  --grpc-port=<port>            gRPC server port for Controller (Default: 50051)
...
# Resolve Interface for Collector
if [[ "$ROLE" == "collector" && -z "$INTERFACE" ]]; then
  INTERFACE="$(ip -4 route show default 2>/dev/null | awk '{print $5}' | head -n1 || true)"
  if [[ -z "$INTERFACE" ]]; then
    for cand in eth0 enp3s0 eno1 wlan0; do
...
TARGET_BIN="${BIN_DIR}/copsec-${ROLE}"

# Check for pre-built binaries or compile on the fly
if [[ -f "${SRC_ROOT}/bin/copsec-${ROLE}" ]]; then
  log_info "Utilizing pre-compiled binary: ${SRC_ROOT}/bin/copsec-${ROLE}"
  cp -f "${SRC_ROOT}/bin/copsec-${ROLE}" "$TARGET_BIN"
elif [[ -f "./bin/copsec-${ROLE}" ]]; then
  log_info "Utilizing pre-compiled binary: ./bin/copsec-${ROLE}"
  cp -f "./bin/copsec-${ROLE}" "$TARGET_BIN"
else
  log_info "Compiling copsec-${ROLE} from source (${SRC_ROOT}/${ROLE})..."
  export CGO_ENABLED=1
  (cd "${SRC_ROOT}/${ROLE}" && go build -ldflags="-s -w" -o "$TARGET_BIN" .)
fi

chmod 755 "$TARGET_BIN"
ln -sf "$TARGET_BIN" "/usr/local/bin/copsec-${ROLE}"

# Compile eBPF Bytecode if role is collector
...
log_success "Binary copsec-${ROLE} prepared and installed to ${TARGET_BIN}."
...
if [[ "$ROLE" == "controller" ]]; then
  # 5A. Initialize SQLite Immutable WAL Ledger
  log_info "Initializing Banking-Grade SQLite Ledger at ${DB_PATH}..."
...
SQL_EOF
  ...
  # Configure Systemd Unit for Controller
...
SYSTEMD_EOF
  ...
  # 5B. Provision Whitelist Configuration
...
WHITELIST_EOF
  ...
  # Provision Node Identity if not present
...
if systemctl is-active --quiet "copsec-${ROLE}.service"; then
  log_success "Unit copsec-${ROLE}.service is ACTIVE and running."
else
  log_warn "Unit copsec-${ROLE}.service status pending. Recent journal entries:"
  journalctl -u "copsec-${ROLE}.service" -n 10 --no-pager || true
fi
