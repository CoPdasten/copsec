...
func NewFleetClient(cfg FleetClientConfig, idMgr *IdentityManager) *FleetClient {
...
func (fc *FleetClient) executeCommand(cmd *fleetproto.ControllerCommand) {
	...
	case fleetproto.ControllerCommand_COMMAND_ENFORCE_BAN:
		...
		// 1. Cross-platform quarantine driver (Linux iptables or Windows netsh firewall)
...
func (fc *FleetClient) collectHealth() *fleetproto.AgentHealth {
	...
	activeBans := 0
	if ebpfEngine := ebpf.GetXDPEngine(); ebpfEngine != nil {
		activeBans = ebpfEngine.GetActiveBansCount()
	}
	if activeBans == 0 {
